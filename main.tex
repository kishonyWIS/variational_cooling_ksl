...
To probe the convergence to the ground state, we compute the finite size spectral Chern number $\nu$ of the single-particle density matrix $R$ of the system fermions, where $R_{s,s'}(\bm{k})=\left\langle c_{(\bm{k},s)}^{z\dagger} c_{(\bm{k},s')}^{z}\right\rangle$. We use the Fukui-Hatsugai-Suzuki method~\cite{Fukui_2005}, applied to the nearest pure Slater determinant obtained by rounding eigenvalues of $R(\bm{k})$ to $0$ or $1$ with threshold $0.5$. From the resulting occupied eigenvectors $\Psi(\bm{k})$, we compute overlap matrices $M_x(\bm{k}) = \Psi^\dagger(\bm{k}) \Psi(\bm{k}+\delta\bm{k}_x)$ and $M_y(\bm{k}) = \Psi^\dagger(\bm{k}) \Psi(\bm{k}+\delta\bm{k}_y)$, where $\delta\bm{k}_x$ and $\delta\bm{k}_y$ are the grid spacings. We extract U(1) link variables $U_\mu(\bm{k}) = \det(\tilde{M}_\mu)/|\det(\tilde{M}_\mu)|$, where $\tilde{M}_\mu$ is the unitary part of $M_\mu$ obtained via polar decomposition. The plaquette flux is then $\Phi_{\square}(\bm{k}) = \arg\left[U_x(\bm{k}) \, U_y(\bm{k}+\delta\bm{k}_x) \, U_x^*(\bm{k}+\delta\bm{k}_y) \, U_y^*(\bm{k})\right]$, and the Chern number is $\nu = (2\pi)^{-1}\sum_{\bm{k}} \Phi_{\square}(\bm{k})$.

This method converges to the integer Chern number as the momentum grid is refined, provided there is a clear gap between occupied and empty bands in the density matrix spectrum. For mixed states, it yields a continuous value that approaches the quantized integer $\nu=1$ as the state approaches the ground state of the KSL model.
